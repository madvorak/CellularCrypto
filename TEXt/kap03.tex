...
Bylo vytvoøeno nìokolik rùznıch algoritmù na protahování klíèù vyuívajících celulární automaty. Je moné si zvláš zvolit algoritmus (instanci tøídy implementující rozhraní IKeyExtender) a zvláš do nìj vloit libovolnı binární celulární automat (instanci tøídy implementující rozhraní IBinaryCA). Nìkteré z agoritmù pøímo generují dlouhı klíè zadané délky -- ty dìdí od abstraktní tøídy KeyExtenderAbstractN. Jiné algoritmy vdy prodlouí klíè na dvojnásobek a obecné nataení realizují iterací tohoto postupu -- ty jsou odvozené od abstraktní tøídy KeyExtenderAbstractD.
...
Vidíme, e pouitı automatù s monotónním chováním vede k pøíšernì špatnım vısledkùm (napøíklad viz \ref{Q220}). Fraktálnì na tom také nejsou dobøe (tøeba viz \ref{Q90}), ale automat èíslo 30, kterı vykazuje pseudonáhodné chování, dává opravdu vynikající vısledek (viz \ref{Q30}). Pouití náhodného (i kdy jinak kvalitnì zvoleného) automatu s 2-okolím vede jen k prùmìrnému vısledku (jako tøeba na obrázku \ref{Qwide}).
